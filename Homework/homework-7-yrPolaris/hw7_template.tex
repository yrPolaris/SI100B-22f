...
\begin{center}
	...
	\vspace{0.1cm}
	\makebox[\textwidth]{Chinese Name:Cao yourong\hrulefill}\\[0.6cm]
	\makebox[\textwidth]{Pinyin Name:Cao yourong\hrulefill}\\[0.6cm]
	\makebox[\textwidth]{Student ID:2022533062\hrulefill}\\[0.6cm]
	\makebox[\textwidth]{E-Mail (omit ``@shanghaitech.edu.cn''):caoyr2022@shanghaitech.edu.cn\hrulefill}\\[0.5cm]
...
\begin{questions}
	...
	\begin{parts}
		...
		\newline Answer:
		\newline Since the Page Size is 16KB
		\newline So the bits used to address virtual pages is $32 - 14 = 18$
		\newline So the number of pages can be addressed is $2^{18}$.
		\newline
		...
		\newline Answer:
		\newline Since 8 bits are reserved for protection and valid bit.
		\newline So bits used for address physical memory is $32 - 8 = 24$
		\newline Since each Page size is 16KB
		\newline So the maximum size of the physical memory that can be supported by this computer is $2^{24+14} = 2^{38}$ byte.
		...
		\newline Answer:
		\newline We should use $\frac{300 * 2^{20}}{2^{14}}$ pages to run the program, it means we need 19200 PTEs to get these pages.
		\newline since these PTEs also need to get, while each page has $\frac{2^{14}}{4} = 4096$ entries, they will occupy $\frac{19200}{4096}$ pages(round to 5, more if not enough) i.e. 5 PTPs.
		\newline
		\newline So the answer is 19200 PTEs and 5 PTPs.
	\end{parts}
	...
	\begin{parts}
		...
		PPN:\quad\underline{8}\quad bits \\

		Page offset:\quad\underline{8}\quad bits\\
	...
	\begin{parts}
		...
		\begin{table}[H]
			...
			\begin{tabular}{cccc}
				...
				\multicolumn{1}{|c|}{2 bits}   & \multicolumn{1}{c|}{2 bits}     & \multicolumn{1}{c|}{4 bits}       \\ \hline
			\end{tabular}
		\end{table}

		\begin{solution}
			Because the Page size is 16 bytes, the number of offset bit is $log_{2}(16) = 4$.\\
			Then for each Page Size, there are 4 PTEs(assume each Page fits one Page table), L1 is as the same as L2, then for Index bit is $log_{2}(4) = 2$.\\
		\end{solution}
		\newpage
